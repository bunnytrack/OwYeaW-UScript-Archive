//=============================================================================
// BT_ScoreBoard made by OwYeaW
// Modified Scoreboard taken from Rush's BT++
//=============================================================================
...
class BT_ScoreBoard extends UnrealCTFScoreBoard;
...
function ShowScores( canvas Canvas )
{
	...
	for ( I=0; I<PlayerCount; I++ )
	{
		if ( Ordered[I].Team < 4 )
		{
			...
			YOffset = ScoreStart + (LoopCountTeam[Ordered[I].Team] * YL * 1.2) + 2;
...
function DrawCapsAndTime(Canvas Canvas, int Caps, string Time, float XOffset, float YOffset, bool bCompressed, int height)
{
	...
	if(Canvas.ClipX <= 800)
		Canvas.SetPos(XOffset + (Canvas.ClipX*0.30)/* - XL/2*/ - XL2, YOffset+YL + (YL2*2));
	else
		Canvas.SetPos(XOffset + (Canvas.ClipX*0.26)/* - XL/2*/ - XL2, YOffset+YL + (YL2*2));
...
{
	local float XL, YL, XL2, YL2, YB, XXL, YYL;
...
/*		else
	...
	if ( Canvas.ClipX > 512 )
	{
		...
		Canvas.DrawColor = WhiteColor;
			...
			if (Canvas.ClipX >= 640)
			{
				if(!bCompressed)
				{
					...
					Canvas.SetPos(XOffset - XL - 6, YOffset + 1);
				...
				{
					// Flag icon
					Canvas.SetPos(XOffset - XL - 56, YOffset);
					Canvas.DrawIcon(FlagIcon[CTFFlag(PRI.HasFlag).Team], 1.6);
				}
				else if(RI.bReadyToPlay && PRI.bWaitingPlayer)
				{
					...
					Canvas.SetPos(XOffset - XL - 56, YOffset);
					Canvas.DrawIcon(FlagIcon[2], 1.6); // draw a green flag
				}
			}
		...
		if (Level.NetMode != NM_Standalone)
		{
			if(!bCompressed)
			{
				...
				Canvas.SetPos(XOffset - XL2 - 6, YOffset + 3 + 2*(YL+1));
				DrawShadowText(Canvas, "L:"$PRI.PacketLoss$"%", false, true);

				if (Canvas.ClipX > 640)
				{
					//	DRAW NETSPEED
					Canvas.StrLen("N:     ", XL2, YL2);
					Canvas.SetPos(XOffset - XL2 - 6, YOffset + 5 + 4*(YL+1));
					if(xBTPRI != None)
						DrawShadowText(Canvas, "N:"$xBTPRI.NetSpeed, false, true);
					else
						DrawShadowText(Canvas, "N:0", false, true);
				}
			}
		}
        if(Canvas.ClipX > 640)
        {
            ...
            Canvas.SetPos(XOffset - XL2 - 6, YOffset + 4 + 3*(YL+1));
            if(RI.Runs > 0)
	...
	if (!bCompressed)
	{
		...
		Canvas.StrLen("Personal Best: ", XXL, YYL);
...
//-------------------------------------------
		...
		Canvas.SetPos(XOffset, YOffset + YB - 2);
...
//-------------------------------------------
		...
		DrawShadowText(Canvas, "Personal Best: ", False, true);
		Canvas.SetPos(XOffset + XXL, YOffset + YB + YL - 1);
		if(xBTPRI != None)
		{
			if(xBTPRI.PersonalBest == "-:--")
			{
				Canvas.DrawColor = SilverColor;
				DrawShadowText(Canvas, xBTPRI.PersonalBest, False, true);
			}
			else
			{
				Canvas.DrawColor = GreenColor;
...
//-------------------------------------------
		// DRAW TIMER
		OldColor = Canvas.DrawColor;
		Canvas.SetPos(XOffset, YOffset + YB + (YL*2));
		DrawShadowText(Canvas, "TIMER: ", False, True);
		Canvas.SetPos(XOffset + XL, YOffset + YB + (YL*2));
		//not on a run?
		...
		if(Canvas.ClipX <= 800)
			Canvas.SetPos(XOffset + (Canvas.ClipX*0.15) - ((XL*3)>>3), YOffset + YB + (YL*2));
		else
			Canvas.SetPos(XOffset + (Canvas.ClipX*0.13) - ((XL*3)>>3), YOffset + YB + (YL*2));
...
//searches for the BT_PRI by the UT PRI given
function BT_PRI FindBTPRI(PlayerReplicationInfo PRI)
{
   local int i;
   local BT_PRI xBTPRI;
	...
	for (i = 0; i < Index; i++)
		if(PI[i].BTPRI != None)
			if(PI[i].BTPRI.PlayerID == PRI.PlayerID)
				return PI[i].BTPRI;

	// Not initialized, find the BT_PRI and init on the correct slot
	foreach Level.AllActors(class'BT_PRI', xBTPRI)
	{
		if(PRI.PlayerID == xBTPRI.PlayerID)
		{
			for(i = 0; i < Index; i++)
			{
				if(PRI == PI[i].PRI)
