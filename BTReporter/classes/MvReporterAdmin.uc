...
function GetReporterClass()
{
  ...
  foreach Level.AllActors(class'MvReporter', lTemp)
    {
      if (String(lTemp.Class) == "BTReporter.MvReporter")
	      {
	        Controller = lTemp;
...
{
  local string colGen, colRed, colTime, colHead, colBody, colGrey, colBlue, colGreen, colGold, colNew;
  ...
  colGrey = Request.GetVariable("colGrey", conf.colGrey);
  ...
  colNew = Request.GetVariable("colNew", conf.colNew);
  ...
  if (Request.GetVariable("Apply", "") == "Apply Settings")
    {
      ...
      conf.colGrey = Controller.IRCLink.ParseDelimited(colGrey, " ", 1);
      ...
      conf.colNew = Controller.IRCLink.ParseDelimited(colNew, " ", 1);
