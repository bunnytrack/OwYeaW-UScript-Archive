  ...
  if (sDestNick ~= ReporterNick)
    {
      ...
      if ((sParams[0] ~= "say") || (sParams[0] ~= "msg"))
		if (CheckPassword(sParams[1], sOrigNick))
		  BroadCastMessage(sOrigNick$": "$sParams[2]);
	...
	if (CheckPassword(sParams[1], sOrigNick))
	  {
	    if (!conf.bMuted)
	      self.SendMessage(conf.colGreen$"*** "$conf.colHead$"Output has been muted by "$sOrigNick);
	    conf.bMuted = !conf.bMuted;
	    conf.SaveConfig();
	    if (!conf.bMuted)
	      self.SendMessage(conf.colGreen$"*** "$conf.colHead$"Output has been un-muted by "$sOrigNick);
	  }
	...
	if (CheckPassword(sParams[1], sOrigNick)) {
	  if (!conf.bPublicComs)
	    self.SendMessage(conf.colGreen$"*** "$conf.colHead$"Public Commands have been enabled by "$sOrigNick);
	  conf.bPublicComs = !conf.bPublicComs;
	  conf.SaveConfig();
	  if (!conf.bPublicComs)
	    self.SendMessage(conf.colGreen$"*** "$conf.colHead$"Public Commands have been disabled by "$sOrigNick);
	}
      ...
      if (CheckPassword(sParams[1], sOrigNick))
	{
	  ...
	  BroadcastMessage("*** "$sOrigNick$" switches map to "$sParams[2]);
	...
	if ((sDestNick ~= conf.Channel) && (!conf.bMuted) && (conf.bPublicComs))
    {
        ...
        if (conf.bUsetBind)
        {
            ...
            if (sParams[0] == conf.tBindSay)
            {
                ...
                {
                    if (conf.xCensorText)
                        CensorTextSrv(sOrigNick$": "$Lower(sParams[1]));
                    else
                    {
                        if (conf.xAllowShouting)
                            BroadCastMessage(sOrigNick$": "$sParams[1]);
                        else
                            BroadCastMessage(sOrigNick$": "$Lower(sParams[1]));
                    }
                }
                else
        ...
        else
        {
            ...
            if (sParams[0] == "!sayx")
				BroadcastMessage(sOrigNick $ ": " $ sParams[1], true, 'CriticalEvent');

            if (sParams[0] == "!s")
				BroadcastMessage(sParams[1], true, 'CriticalEvent');
