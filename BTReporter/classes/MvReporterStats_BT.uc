...
function InLocalizedMessage( class<LocalMessage> Message, optional int Switch, optional PlayerReplicationInfo RelatedPRI_1, optional PlayerReplicationInfo RelatedPRI_2, optional Object OptionalObject )
{
  ...
  if (ClassIsChildOf(Message, class'BotPack.DeathMatchMessage'))
      ...
      switch(Switch)
	{
	  // Overtime :)
	case 0:
	  SendIRCMessage(GetColoredMessage("", conf.colGreen, Message, Switch, RelatedPRI_1, RelatedPRI_2, OptionalObject));
	  return;
	  // Team Change
	case 3:
	  SendIRCMessage(GetColoredMessage("* ", conf.colRed, Message, Switch, RelatedPRI_1, RelatedPRI_2, OptionalObject));
	  return;
	}
    }
  ...
  if (ClassIsChildOf(Message, class'BotPack.CTFMessage'))
	...
	case 0:
	  SendIRCMessage(conf.colRed$ircUnderline$Message.static.GetString( Switch, RelatedPRI_1, RelatedPRI_2, OptionalObject));
	...
	case 2:
	  ...
	  droppedMessage = conf.colRed$Message.static.GetString( Switch, RelatedPRI_1, RelatedPRI_2, OptionalObject);
	  break;
	default:
	  SendIRCMessage(conf.colRed$Message.static.GetString( Switch, RelatedPRI_1, RelatedPRI_2, OptionalObject));
	  break;
	}
    }
    if (Switch==0)
    {
	  DiedMsg="";
	  droppedMessage="";
    	  SendIRCMessage(conf.colRed$ircUnderline$GetTeamColor(RelatedPRI_1.Team)$RelatedPRI_1.PlayerName$ircClear$conf.ColHead$ircUnderline@"has captured the flag !"$ircClear$conf.ColHead$" (Best Time: "$ProcessScore(RelatedPRI_1.Score)$")");
	  SendScoreLine("New Score: ");
	  return;
    }
}
...
function OnScoreDetails()
{
  ...
  if (bBTScores && bestPRI.Score!=0)
	  SendIRCMessage(">> " $ conf.colHead $ bestPRI.PlayerName $ " has the best CapTime -"$conf.colGreen$" "$ProcessScore(bestPRI.Score)$conf.colHead$" !");
}
...
{
  local int iScore[4];
  SendIRCMessage(conf.colRed$sPreFix$GetTeamColor(0)$conf.sTeams[0]$" "$string(int(TeamGamePlus(Level.Game).Teams[0].Score))$ircColor$":"$GetTeamColor(1)$ircBold$ircBold$string(int(TeamGamePlus(Level.Game).Teams[1].Score))$" "$conf.sTeams[1]);
}
...
function SendScoreBoard(string sHeadLine, optional bool bTime)
{
  ...
  SendIRCMessage(conf.colRed$sHeadLine, bTime);
