...
function PostPlayerStats()
{
  ...
  if (Link.bUTGLEnabled)
    SendIRCMessage(conf.colBody $ PostPad("Name", 22, " ") $ "| " $ PrePad("Login", 15, " ") $ "| " $ PrePad(sScoreStr, 5, " ") $ " | " $ PrePad("Death", 5, " ") $ " | " $ PrePad("Ping", 4, " ") $ " | " $ PrePad("PL", 4, " ") $ " |", TRUE);
  else
    SendIRCMessage(conf.colBody $ PostPad("Name", 22, " ") $ "| " $ PrePad(sScoreStr, 5, " ") $ " | " $ PrePad("Death", 5, " ") $ " | " $ PrePad("Ping", 4, " ") $ " | " $ PrePad("PL", 4, " ") $ " |", TRUE);
...
function OnScoreDetails()
{
  ...
  // Head
  SendIRCMessage(" ");
  SendIRCMessage(conf.colRed$"** Team Status Information:");
  ...
  SendIRCMessage(conf.colHead$"Best Player is"$GetTeamColor(bestPRI.Team)$" "$bestPRI.PlayerName$conf.colHead$" with"$conf.colGreen$" "$string(int(bestPRI.Score))$conf.colHead$" Frags!");
  SendIRCMessage(" ");
}
