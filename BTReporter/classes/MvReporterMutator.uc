...
function Mutate(string MutateString, PlayerPawn Sender)
{
	...
	if (Sender.bAdmin)
	{
		if (Left(MutateString, 4) ~= "mvr ")
		{
			...
			{
				if (!conf.bMuted)
					Link.SendMessage(conf.colGreen$"*** "$conf.colHead$"Output has been muted.");
				conf.bMuted = !conf.bMuted;
				conf.SaveConfig();
				if (!conf.bMuted)
					Link.SendMessage(conf.colGreen$"*** "$conf.colHead$"Output has been un-muted.");
			...
			{
				if (!conf.bPublicComs)
					Link.SendMessage(conf.colGreen$"*** "$conf.colHead$"Public Commands have been enabled.");
				conf.bPublicComs = !conf.bPublicComs;
				conf.SaveConfig();
				if (!conf.bPublicComs)
					Link.SendMessage(conf.colGreen$"*** "$conf.colHead$"Public Commands have been disabled.");
				Sender.ClientMessage("Applied.");
			}
