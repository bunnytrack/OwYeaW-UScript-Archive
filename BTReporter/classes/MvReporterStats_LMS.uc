...
function OnGameDetails()
{
  ...
  SendIRCMessage("> " $ GetTeamColor(bestPRI.Team) $ bestPRI.PlayerName $ conf.colHead $ " is in the lead with"$conf.colGreen$" "$string(int(bestPRI.Score))$conf.colHead$" lives left!");
  SendIRCMessage(" ");
}
